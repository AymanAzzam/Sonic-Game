...
void Application::setScene(GameScene* scene){
...
void Application::run(){
    ...
    while(!glfwWindowShouldClose(window) && !glfwGetKey(window, GLFW_KEY_ESCAPE)){
		...
		if (!pause) //when pause the game stop the time
			timestamp++;

        double currentFrame = glfwGetTime();
        
		if (currentScene->Simulation(currentFrame - lastFrame, timestamp, pause))//Send time difference between last and current frame
		{
			timestamp = 0;
			currentScene->InitializeData();
		}
